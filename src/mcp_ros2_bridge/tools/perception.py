...
def get_perception_tools() -> list[Tool]:
    """Get list of perception-related MCP tools."""
    return [
        Tool(
...
async def handle_perception_tool(
    ...
    if name == "get_obstacle_distances":
        ...
        result = _get_obstacle_distances(
            state.scan_ranges, directions, state.scan_angle_min, state.scan_angle_increment
        )

    elif name == "check_path_clear":
        distance = arguments.get("distance_meters", 1.0)
        width = arguments.get("width_meters", 0.5)
        result = _check_path_clear(
            state.scan_ranges, distance, width, state.scan_angle_min, state.scan_angle_increment
        )

    elif name == "scan_surroundings":
        num_sectors = arguments.get("num_sectors", 8)
        result = _scan_surroundings(
            state.scan_ranges,
            num_sectors,
...
def _scan_surroundings(
    ...
    for i in range(num_sectors):
        ...
        sector_ranges = [
            r for r in ranges[start_idx : end_idx + 1] if not math.isinf(r) and not math.isnan(r)
        ]
