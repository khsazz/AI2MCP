...
def register_motion_tools(server: Server, ros_bridge: ROS2Bridge) -> None:
    """Register motion-related MCP tools."""

    @server.list_tools()
    async def list_motion_tools() -> list[Tool]:
        """List available motion tools."""
        return [
            Tool(
                name="move",
                description=(
    ...
    @server.call_tool()
    async def call_motion_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
