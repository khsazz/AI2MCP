...
class PredictionToolsManager:
    ...
    def get_world_graph(self, frame_idx: int | None = None) -> dict:
        ...
        # Run through GNN to get embeddings
        self.gnn_model.eval()
        with torch.no_grad():
        ...
        return {
            "graph": graph_json,
            "node_embedding_dim": outputs["node_embeddings"].shape[-1],
            "graph_embedding": outputs["graph_embedding"].squeeze().tolist(),
...
def register_prediction_tools(
    ...
    Args:
        ...
        tools_manager: PredictionToolsManager for GNN predictions
    """
    existing_list = server._tool_handlers.get("list_tools")

    @server.list_tools()
    async def list_prediction_tools() -> list[Tool]:
        """List prediction tools."""
        existing_tools = await existing_list() if existing_list else []
        ...
        prediction_tools = [
            Tool(
                ...
                description=(
                    ...
                    "and graph-level embeddings for reasoning."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        "frame_idx": {
                            "type": "integer",
                            "description": (
                                "Frame index to analyze. If not provided, uses current frame."
                            ),
                            "minimum": 0,
                        },
                    },
        ...
        return existing_tools + prediction_tools

    @server.call_tool()
    async def call_prediction_tool(
        ...
        if name == "get_world_graph":
            ...
            result = tools_manager.get_world_graph(frame_idx)
