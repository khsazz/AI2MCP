...
The architecture implements a multi-layered data flow designed to resolve the $N \times M$ integration problem \cite{zeng_m3llm_2025}. As illustrated in \Cref{fig:architecture}, the system is partitioned into two primary control loops:
\begin{itemize}
    \item \textbf{Reflexive Loop (ROS 2)}: Operates at $>100$ Hz, handling hardware stability, joint-level actuation, and sensor fusion via DDS.
    \item \textbf{Cognitive Loop (MCP)}: Operates at $\sim16.9$ Hz, handling high-level reasoning, tool selection, and semantic state estimation via JSON-RPC.
\end{itemize}

This separation ensures that modifications to the AI ``brain'' or the robotic ``body'' do not require a full-stack reconfiguration \cite{liskova_cps-based_2025, imran_safety-critical_2025, zhou_multi-robot_2022}:
...
We chose the Model Context Protocol (MCP) over direct ROS~2 APIs to ensure the agent remains agnostic to DDS-level message structures, preventing 'callback bloat' at the cognitive layer. Furthermore, we utilize a GNN over a standard MLP because GNNs provide inherent permutation invariance over the joint-chain entities, ensuring the model generalizes to different arm configurations.
...
\end{equation}
where $(x, y, z)$ is the 3D position, $\theta$ is the joint angle, and $t$ is a one-hot joint type encoding. The input to the network is a set of 16 nodes (14 joints + 2 gripper bases), and the output is a set of 9 binary predicates for each edge in the fully connected graph.
...
To address LLM non-determinism, a \textbf{ForwardDynamicsModel} performs rollouts to verify plans \cite{roth_learned_2025, nguyen_model-free_2024, fisac_general_2017, molnar_model-free_2021, liang_point_2025, imran_safety-critical_2025}. The Forward Dynamics Model acts as a 'Physicality Filter,' predicting future states with an error of $\delta = 0.0017$. It intercepts commands that lead to kinematically impossible states, returning a 'REPLAN' recommendation to the LLM agent.
