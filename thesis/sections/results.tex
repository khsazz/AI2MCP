...
The Kinematic-GNN reasoning engine achieved a validation accuracy of \textbf{97.03\%} during training. As illustrated in \Cref{fig:training-curves}, the model exhibits rapid convergence with minimal loss variance, reaching optimal performance by epoch 69.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/training_curves.png}
    \caption{Kinematic-GNN training on 55,000 ALOHA frames showing high stability in both loss convergence and micro-accuracy.}
    \label{fig:training-curves}
\end{figure}

\subsection{Inference Benchmark Results}
The trained Kinematic-GNN was evaluated on a 500-frame holdout set, demonstrating substantial improvements over locally-trained baselines. Table~\ref{tab:inference-benchmark} summarizes the inference performance.
...
To emphasize the architectural advantages of a protocol-driven GNN over heavy multimodal stacks, we utilize a radar chart to compare the Kinematic-GNN and the Multimodal-GNN.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/radar_comparison.png}
    \caption{Radar chart evaluating the trade-offs between kinematic reasoning and multimodal fusion. The Kinematic-GNN dominates on latency, memory, and model size.}
    \label{fig:radar-comparison}
\end{figure}

\section{Detailed Comparative Metrics}

\subsection{Fair Architecture Comparison (Kinematic vs. Multimodal)}
...
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Kinematic-GNN (A)} & \textbf{Multimodal-GNN (C)} & \textbf{Winner} \\
\midrule
Micro Accuracy & \textbf{97.03\%} & 96.51\% & \textbf{A} \\
Inference Latency & \textbf{1.52ms} & 24.29ms & \textbf{A ($16\times$)} \\
Peak Memory (MB) & \textbf{19.4} & 141.8 & \textbf{A ($7\times$)} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Perception Bottlenecks}
While the MCP overhead is minimal (0.6ms), the honest E2E latency (297--332ms) is dominated by visual detection (234ms), establishing a real-world stability ceiling for protocol-driven control.

\subsection{Per-Predicate Distribution and the Interaction Gap}
Detailed analysis of the predicate distribution reveals high fidelity in spatial predicates like \texttt{is\_near} (F1: 0.954) and \texttt{is\_left\_of} (F1: 0.969). However, an ``Interaction Predicate Gap'' exists for contact-based predicates (\texttt{is\_holding}), which reported 0.000 F1 on real data due to a lack of annotated contact signals in the kinematic stream. Notably, when trained on synthetic data with augmented holding scenarios, the architecture achieved an F1 of \textbf{0.914} for \texttt{is\_holding}, demonstrating that the limitation is driven by dataset characteristics rather than architectural deficiency.
...
\begin{figure}[htbp]
    ...
    \caption{Summary of predictive stability and dynamics performance across the proposed temporal reasoning architectures.}
...
\begin{itemize}
    ...
    \item \textbf{Computational Advantage}: Kinematic-GNN is $16\times$ faster and $7\times$ leaner than vision-fusion alternatives.
